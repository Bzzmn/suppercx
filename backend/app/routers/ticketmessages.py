...
async def create_ticket_message(
    ticket_message: TicketMessageCreate, db: AsyncSession = Depends(get_db)
):
...
async def read_ticket_message(message_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(TicketMessage).filter(TicketMessage.id == message_id)
    )
...
async def update_ticket_message(
    message_id: int,
    ticket_message: TicketMessageUpdate,
